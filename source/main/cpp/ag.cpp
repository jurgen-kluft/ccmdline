...
namespace xcore
{
	namespace xcmdline
	{
		...
		*/
		...
		xargv* Ag_Func::ag_new(s32 argc, char **argv)
		{
			xargv *ag;
			ag = (xargv *)Opt_Allocator::get_opt_allocator()->allocate(sizeof(xargv), 4);
		...
		void Ag_Func::ag_free(xargv *ag)
		{
			Opt_Allocator::get_opt_allocator()->deallocate((char *)ag);
		}

		void Ag_Func::ag_reset(xargv *ag)
		{
			ag->iw = 0;
			ag->ic = 0;
		}

		s32 Ag_Func::ag_w_number(xargv *ag)
		{
			return ag->iw;
		}

		void Ag_Func::ag_w_advance(xargv *ag)
		{
		...
		*/
		...
		do not advance
		*/
		char Ag_Func::ag_c(xargv *ag)
		{
			return(ag->v[ag->iw][ag->ic]);
		}

		char Ag_Func::ag_c_next(xargv *ag)
		{
			return(ag->v[ag->iw][ag->ic+1]);
		}

		char Ag_Func::ag_cnn_next(xargv *ag)
		{
			if( ag_c_next(ag) == '\0' )
			{
				if(ag->iw+1 >= ag->c)
		...
		*/
		char Ag_Func::ag_c_advance(xargv *ag)
		{
		...
		*/
		...
		char*	Ag_Func::ag_s_next(xargv *ag)
		{
		...
		*/
		char*	Ag_Func::ag_s_advance(xargv *ag)
		{
		...
		s32		Ag_Func::ag_argc(xargv *ag)
		{
			return ag->c - ag->iw;
		}

		char**	Ag_Func::ag_argv(xargv *ag)
		{
			return ag->v + ag->iw;
		}

		xargv*	Ag_Func::ag_copy(xargv *ag)
		{
			/* Here is how you would make a copy of an xargv */
			/* To free this, do not use ag_free, but ag_freeall */
			s32 i;
			xargv *agnew;
			agnew = (xargv *)Opt_Allocator::get_opt_allocator()->allocate(sizeof(xargv), 4);
			agnew->c = ag->c;
			agnew->v = (char **)Opt_Allocator::get_opt_allocator()->allocate(sizeof(char *) * agnew->c, 4);
			for (i=0; i< agnew->c; ++i) 
			{
				agnew->v[i] = Opt_Allocator::opt_strdup(ag->v[i]);
			}
			agnew->iw = 0;
			agnew->ic = 0;
			return agnew;
		...
		void	Ag_Func::ag_freeall(xargv *ag)
		{
			s32 i;
			for (i=0; i<ag->c; ++i)
			{
				Opt_Allocator::get_opt_allocator()->deallocate(ag->v[i]);
			}
			Opt_Allocator::get_opt_allocator()->deallocate((char *)ag->v);
			Opt_Allocator::get_opt_allocator()->deallocate((char *)ag);
		}

		xargv*	Ag_Func::ag_prepend_argv0(xargv *ag, char *argv0)
		{
			s32 i;
			xargv *agnew;
			agnew = (xargv *)Opt_Allocator::get_opt_allocator()->allocate(sizeof(xargv), 4);
			agnew->c = ag->c - ag->iw + 1;
			agnew->v = (char **)Opt_Allocator::get_opt_allocator()->allocate(agnew->c * sizeof(char *), 4);
